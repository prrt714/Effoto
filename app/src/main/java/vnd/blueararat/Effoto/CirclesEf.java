...
public class CirclesEf extends Effect {
	...
	public CirclesEf(Context ctx) {
		this(ctx, 0.01f + RANDOM.nextFloat()/10);
		// this(ctx,
	...
	public CirclesEf(Context ctx, float lscale) {
		this(ctx, lscale, 0.25f / lscale + RANDOM.nextFloat()/(4*lscale),
				255, 0, 0, 0, 0.6f + RANDOM.nextFloat()*0.8f, 0.6f + RANDOM.nextFloat()*0.8f, 0, null);
	}
	...
	private synchronized void draw(Bitmap bitmap) {
		...
		for (int i = 0; i < bh; i++) {
			...
			for (int j = 0; j < bw; j++) {
				...
				p.setAlpha(opacity);
				// p.setAlpha(125);
				c.drawCircle(x + rnd_pos_max * radius * RANDOM.nextFloat(), y
						+ rnd_pos_max * radius * RANDOM.nextFloat(), radius
						+ rnd_size_max * radius * (0.5f - RANDOM.nextFloat()),
						p);
